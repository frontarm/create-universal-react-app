...
cd /var/create-universal-react-app
...
git stash pop
cd -
mv /var/create-universal-react-app/patch .
git apply patch
rm patch
CMD
...
set -x
git clone /var/create-universal-react-app create-universal-react-app --branch ${git_branch}
cd create-universal-react-app
${apply_changes}
node --version
npm --version
set +x
...
docker run \
  ...
  --rm \
  --user node \
  --volume ${PWD}/..:/var/create-universal-react-app \
  --workdir /home/node \
